...
Elm.Main.make = function (_elm) {
   ...
   var mouseToPos = F2(function (_v0,
   _v1) {
      return function () {
         switch (_v1.ctor)
         {case "_Tuple2":
   ...
   var puckYellow = A3($Color.rgb,
   250,
   250,
   200);
   var goalRed = A3($Color.rgb,
   240,
   10,
   95);
   ...
   string) {
      return $Graphics$Element.leftAligned(f($Text.monospace($Text.color(textBlue)($Text.fromString(string)))));
   });
   var rinkBlue = A3($Color.rgb,
   15,
   185,
   215);
   var capVelocity = F2(function (maxV,
   _v13) {
      return function () {
         switch (_v13.ctor)
         {case "_Tuple2":
            return function () {
                 var maxV2 = maxV * maxV;
                 var absV2 = _v13._0 * _v13._0 + _v13._1 * _v13._1;
                 return _U.cmp(maxV2,
                 absV2) > 0 ? {ctor: "_Tuple2"
                              ,_0: _v13._0
                              ,_1: _v13._1} : function () {
                    var scale = $Basics.sqrt(maxV2 / absV2);
                    return {ctor: "_Tuple2"
                           ,_0: _v13._0 * scale
                           ,_1: _v13._1 * scale};
                 }();
   ...
   var boundBox = {_: {}
                  ...
                  ,shape: $BoxesAndBubblesBodies.Box({ctor: "_Tuple2"
                                                     ,_0: 0
                                                     ,_1: 0})
                  ,velocity: {ctor: "_Tuple2"
                             ,_0: 0
                             ,_1: 0}};
   var inf = 1 / 0;
   var maxVelocity = 20.0;
   var boundsWidth = 10000.0;
   var puckSize = 20.0;
   var defaultPuck = A5($BoxesAndBubbles.bubble,
   puckSize,
   10,
   e0,
   {ctor: "_Tuple2",_0: 0,_1: 0},
   {ctor: "_Tuple2",_0: 0,_1: 0});
   var updatePuck = F3(function (dt,
   puck,
   bodies) {
      return function () {
         var _v29 = $List.head(A3($BoxesAndBubblesEngine.collideWith,
         puck,
         bodies,
         _L.fromArray([])));
         switch (_v29.ctor)
         {case "Just":
            return A3($BoxesAndBubblesEngine.update,
              {ctor: "_Tuple2",_0: 0,_1: 0},
              {ctor: "_Tuple2",_0: 0,_1: 0},
              _U.replace([["velocity"
                          ,A2(capVelocity,
   ...
   var paddleSize = 30.0;
   var getPaddle = F2(function (pos,
   v) {
      return A5($BoxesAndBubbles.bubble,
      paddleSize,
      inf,
      e0,
      pos,
   ...
   var goalHeight = 100.0;
   var halfGoal = goalHeight / 2;
   var $ = {ctor: "_Tuple2"
           ,_0: 600.0
           ,_1: 400.0},
   gameWidth = $._0,
   ...
   var newPlayer = function (side) {
      return {_: {}
             ,paddle: {_: {}
                      ,current: defaultPaddle(side)
                      ,last: defaultPaddle(side)}
             ,score: 0};
   };
   var newPlayer1 = newPlayer(Left);
   var newPlayer2 = newPlayer(Right);
   var defaultGame = {_: {}
                     ,player1: newPlayer(Left)
                     ,player2: newPlayer(Right)
                     ,puck: defaultPuck
                     ,state: Pause};
   var bounds = A2($Basics._op["++"],
   _L.fromArray([_U.replace([["pos"
                             ,{ctor: "_Tuple2"
                              ,_0: 0
                              ,_1: (gameHeight + boundsWidth * 2) / 2}]
                            ,["shape"
                             ,$BoxesAndBubblesBodies.Box({ctor: "_Tuple2"
                                                         ,_0: gameWidth + boundsWidth
                                                         ,_1: boundsWidth})]],
                boundBox)
                ,_U.replace([["pos"
                             ,{ctor: "_Tuple2"
                              ,_0: 0
                              ,_1: (0 - (gameHeight + boundsWidth * 2)) / 2}]
                            ,["shape"
                             ,$BoxesAndBubblesBodies.Box({ctor: "_Tuple2"
                                                         ,_0: gameWidth + boundsWidth
                                                         ,_1: boundsWidth})]],
   ...
   _L.fromArray([_U.replace([["pos"
                             ,{ctor: "_Tuple2"
                              ,_0: (0 - (gameWidth + boundsWidth * 2)) / 2
                              ,_1: (gameHeight + goalHeight) / 4}]
                            ,["shape"
                             ,$BoxesAndBubblesBodies.Box({ctor: "_Tuple2"
                                                         ,_0: boundsWidth
                                                         ,_1: (gameHeight - goalHeight) / 4})]],
                ...
                ,_U.replace([["pos"
                            ...
                            ,["shape"
                             ,$BoxesAndBubblesBodies.Box({ctor: "_Tuple2"
                                                         ...
                                                         ,_1: (gameHeight - goalHeight) / 4})]],
                boundBox)])));
   var clampInGame = F3(function (side,
   size,
   pos) {
      return function () {
   ...
   var updatePlayer = F4(function (dt,
   side,
   pos,
   player) {
      return function () {
         var currentPos = A3(clampInGame,
         side,
         paddleSize,
         pos);
         var last = player.paddle.last;
         var current = player.paddle.current;
         var paddle = {_: {}
                      ,current: _U.replace([["pos"
                                            ,currentPos]
                                           ,["velocity"
                                            ,scale2(1 / dt)(A2(diff2,
                                            current.pos,
                                            last.pos))]],
                      current)
                      ,last: current};
         return _U.replace([["paddle"
                            ,paddle]],
         player);
      }();
   });
   var update = F2(function (_v34,
   _v35) {
      return function () {
         return function () {
            return function () {
               var newPlayer2 = function () {
                  var _v38 = _v35.state;
                  switch (_v38.ctor)
                  {case "Pause":
                     return _v35.player2;}
                  return A4(updatePlayer,
                  _v34.dt,
                  Left,
                  _v34.pos2,
                  _v35.player2);
               }();
               var newPlayer1 = function () {
                  var _v39 = _v35.state;
                  switch (_v39.ctor)
                  {case "Pause":
                     return _v35.player1;}
               ...
               var newState = _v34.space ? Play : _v35.state;
               var score2 = A2(inGoal,
               _v35.puck,
               Left) ? 1 : 0;
               var score1 = A2(inGoal,
               _v35.puck,
               Right) ? 1 : 0;
               var blarg = A2($Debug.watch,
               "hi",
               {_: {}
               ,puck: _v35.puck
               ,score1: score1
               ,score2: score2});
               var newPuck = !_U.eq(score1,
               score2) ? defaultPuck : _U.eq(_v35.state,
               Pause) ? _v35.puck : A3(updatePuck,
               _v34.dt,
               _v35.puck,
               A2($Basics._op["++"],
               _L.fromArray([newPlayer1.paddle.current
                            ,newPlayer2.paddle.current]),
               bounds));
               return {_: {}
                      ,player1: _U.replace([["score"
                                            ,_v35.player1.score + score1]],
                      newPlayer1)
                      ,player2: _U.replace([["score"
                                            ,_v35.player2.score + score2]],
                      newPlayer2)
                      ,puck: newPuck
                      ,state: newState};
            }();
         ...
         return function () {
            ...
            {case "_Tuple2":
               return function () {
                    ...
                    $Text.height(50),
                    A2($Basics._op["++"],
                    $Basics.toString(_v41.player1.score),
                    A2($Basics._op["++"],
                    "  ",
                    $Basics.toString(_v41.player2.score))));
                    return A3($Graphics$Element.container,
                    _v40._0,
                    _v40._1,
                    $Graphics$Element.middle)(A3($Graphics$Collage.collage,
                    $Basics.floor(gameWidth),
                    $Basics.floor(gameHeight),
                    _L.fromArray([$Graphics$Collage.filled(rinkBlue)(A2($Graphics$Collage.rect,
                                 gameWidth,
                                 gameHeight))
                                 ,$Graphics$Collage.filled(textBlue)(A2($Graphics$Collage.rect,
                                 5,
                                 gameHeight))
                                 ,$Graphics$Collage.move({ctor: "_Tuple2"
                                                         ,_0: 2.5 - gameWidth / 2
                                                         ,_1: 0})($Graphics$Collage.filled(goalRed)(A2($Graphics$Collage.rect,
                                 5,
                                 goalHeight)))
                                 ,$Graphics$Collage.move({ctor: "_Tuple2"
                                                         ,_0: gameWidth / 2 - 2.5
                                 ...
                                 5,
                                 goalHeight)))
                                 ,A2(viewBB,_v41.puck,puckYellow)
                                 ,A2(viewBB,
                                 _v41.player1.paddle.current,
                                 $Color.white)
                                 ,A2(viewBB,
                                 _v41.player2.paddle.current,
                                 $Color.white)
                                 ,$Graphics$Collage.move({ctor: "_Tuple2"
                                                         ,_0: 0
                                 ...
                                 ,$Graphics$Collage.move({ctor: "_Tuple2"
                                                         ,_0: 0
                                                         ,_1: 40 - gameHeight / 2})($Graphics$Collage.toForm(_U.eq(_v41.state,
   ...
   var input = $Signal.sampleOn(dt)(A5($Signal.map4,
   Input,
   $Keyboard.space,
   A2($Signal._op["~"],
   A2($Signal._op["<~"],
   ...
   _elm.Main.values = {_op: _op
                      ...
                      ,clampInGame: clampInGame
                      ,updatePlayer: updatePlayer
                      ,view: view
                      ,rinkBlue: rinkBlue
                      ,textBlue: textBlue
                      ,goalRed: goalRed
                      ,puckYellow: puckYellow
                      ,txt: txt
                      ,msg: msg
                      ,viewBB: viewBB
                      ,input: input
